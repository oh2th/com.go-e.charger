...
exports.init = async function(homey) {
  ...
  const action_THREE_PHASE = homey.flow.getActionCard('three_phase');
  action_THREE_PHASE.registerRunListener(async (args, state) => {
    await args.device.onCapability_THREE_PHASE(args.enabled);
    return {
      enabled: args.enabled,
    };
  });
