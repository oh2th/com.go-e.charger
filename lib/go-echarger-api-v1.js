...
module.exports = class GoeChargerApi {
  ...
  async getInfo() {
    const res = await this._getGoeCharger(this.address, 'status'); // go-e status API V1 
  ...
  async setChargeCurrentLimit(vlw) {
    return this._setGoeCharger(this.address, `amp=${vlw}`);
  }
