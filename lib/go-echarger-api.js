...
module.exports = class GoeChargerApi {
	...
	async _getInfoAPIV2() {
        ...
        // output voltage (V2 API pha and nrg arrays)
        let numPhases_out = 0;
        ...
        if (numPhases_out === 3) {
            deviceInfo['measure_voltage.output'] = Number(((deviceInfo['measure_voltage.output'] / numPhases_out) * Math.sqrt(3)).toFixed(1));
        }
        // check for reversed phases.
        if (deviceInfo['measure_voltage.output'] < 0) deviceInfo['measure_voltage.output'] = Number(goecharger.nrg[3].toFixed(1));
