...
module.exports = class GoeChargerApi {
	...
	async _getInfoAPIV1() {
		...
		if (numPhases === 1) {
			...
			// multi phase
			deviceInfo.measure_voltage = Number(((goecharger.nrg[0] + goecharger.nrg[1] + goecharger.nrg[2]) / numPhases * Math.sqrt(3)).toFixed(1));
		}
		...
		numPhases = 0;
		if ((goecharger.pha & 4) === 4) numPhases++;
		if ((goecharger.pha & 2) === 2) numPhases++;
		if ((goecharger.pha & 1) === 1) numPhases++;
		if (numPhases === 0) {
			// no phase
			deviceInfo['measure_voltage.output'] = 0;
		} else if (numPhases === 1) {
			// single phase
			if ((goecharger.pha & 4) === 4) deviceInfo['measure_voltage.output'] = Number((goecharger.nrg[2]).toFixed(1));
			if ((goecharger.pha & 2) === 2) deviceInfo['measure_voltage.output'] = Number((goecharger.nrg[1]).toFixed(1));
			if ((goecharger.pha & 1) === 1) deviceInfo['measure_voltage.output'] = Number((goecharger.nrg[0]).toFixed(1));
		} else {
			// multi phase
			deviceInfo['measure_voltage.output'] = Number(((goecharger.nrg[0] + goecharger.nrg[1] + goecharger.nrg[2]) / numPhases * Math.sqrt(3)).toFixed(1));
		}
		console.log(`[GoeChargerApi] _getInfoAPIV1: numPhases: ${numPhases}, measure_voltage: ${deviceInfo.measure_voltage}`)
	...
	async _getInfoAPIV2() {
		...
		// voltage from output phases (V2 API nrg array)
		numPhases = 0;
		...
		console.log(`[GoeChargerApi] _getInfoAPIV2: numPhases: ${numPhases}, measure_voltage: ${deviceInfo.measure_voltage}`);
		// check for reversed phases.
		if (deviceInfo.measure_voltage < 0) deviceInfo.measure_voltage = Number((goecharger.nrg[3]).toFixed(1));

		// output voltage (V1 API nrg array)
		numPhases = 0;
		if (goecharger.pha[0] === true) numPhases++;
		if (goecharger.pha[1] === true) numPhases++;
		if (goecharger.pha[2] === true) numPhases++;
		if (numPhases === 0) {
			// no phase
			deviceInfo['measure_voltage.output'] = 0;
		} else if (numPhases === 1) {
			// single phase
			if (goecharger.pha[2] === true) deviceInfo['measure_voltage.output'] = Number((goecharger.nrg[2]).toFixed(1));
			if (goecharger.pha[1] === true) deviceInfo['measure_voltage.output'] = Number((goecharger.nrg[1]).toFixed(1));
			if (goecharger.pha[0] === true) deviceInfo['measure_voltage.output'] = Number((goecharger.nrg[0]).toFixed(1));
		} else {
			// multi phase
			deviceInfo['measure_voltage.output'] = Number(((goecharger.nrg[0] + goecharger.nrg[1] + goecharger.nrg[2]) / numPhases * Math.sqrt(3)).toFixed(1));
		}
