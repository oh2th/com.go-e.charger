...
module.exports = class GoeChargerApi {
  ...
  async _getInfoAPIV1() {
    ...
    if (goecharger.eto > 0) deviceInfo.energy_total = Number((goecharger.eto / 10).toFixed(2));
    ...
    deviceInfo.cable_limit = Number(goecharger.cbl);
  ...
  async _getInfoAPIV2() {
    ...
    const res = await this._getGoeCharger('api/status?filter=sse,fna,car,alw,err,wh,pha,nrg,eto,tma,amp,ama,cbl');
    const txt = await res.text();
    const goecharger = JSON.parse(txt);
    ...
    if (goecharger.eto > 0) deviceInfo.energy_total = Number((goecharger.eto / 1000).toFixed(2));
